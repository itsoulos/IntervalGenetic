...
{
	/** CHANGE FOR GRBF METHOD
	 * Relevant paper: https://www.researchgate.net/publication/220372902_An_optimizing_method_of_RBF_neural_network_based_on_genetic_algorithm
	 */
	int icount=0;
    int d=trainSet->getdimension();
    vector<int> flags;
   flags.resize(nweights); 
   for(int i=0;i<nweights;i++) flags[i]=x[i]>0.5?1:0;
   icount = nweights;
   for(int i=0;i<nweights;i++)
   {
	double nextElement = x[icount++];
	if(flags[i]) weight[i]=nextElement; else weight[i]=0.0;
   }
}
