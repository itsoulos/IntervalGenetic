...
void Kmeans(vector<double> &rnd,double * data_vectors, double * centers, 
            double * variances, int m, int n, int K)
{
	int i=0;
	int j=0;
	...
	{
		do{
			double randomNumber = rnd[rand_index++];
			if(rand_index>=rnd.size()) rand_index=0;
            random_centers[i]=(int)((m-1) * randomNumber);
		}while(random_centers[i]>m);
