...
./IntervalGenetic  --localmethod=genetic --intervalmethod=integer --filename=PROBLEMS/Feature/libFeature.so --param=features --value=2 --param=trainName --value=$DATAPATH/$1.train --param=testName --value=$DATAPATH/$1.test --param=nodes --value=$NODES --generations=$GENS  --threads=8
