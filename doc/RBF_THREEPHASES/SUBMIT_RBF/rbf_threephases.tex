...
the external weights for the radial functions are adjusted. However,
in many cases, this training technique has reduced performance either
due to instability in arithmetic operations or the the method's inability
to escape from local minima of the error function. In this manuscript,
a three-stage method is suggested in this work, to address the above
problems. During the execution of the first phase, an initial estimate
of the value interval for the machine learning model parameters is
made. In the second stage, the network parameter values are adjusted
...
as well as the original RBF training method and the current work has
been successfully applied to a large group of related problems from
the recent literature. Also\textbf{, }a comparison was made in terms
of classification or regression error. It should be noted that although
the proposed methodology had very good results in the above measurements,
it requires significant computational execution time, due to the use
of three phases of processing and adaptation of the network parameters.}
...
used machine learning tools, one can detect the Radial Basis Function
(RBF) network. These machine learning models are commonly defined
...
the effective training of RBF networks.\textbf{ }The first phase of
the technique includes an algorithm for efficiently identifying the
value interval of the model parameters. This detection is implemented
using the K-Means algorithm \citep{kmeans} for the weights and the
variances of the radial functions. After applying the above procedure,
a range of values for the network parameters is created which directly
depends on the values produced by K-means algorithm. During the second
stage of the proposed work, a global optimization procedure is incorporated
to optimize the parameters of the RBF network with respect to the
equation \ref{eq:RbfError}. The training of the network parameters
is performed within the value range of the first phase to avoid possible
overfitting problems. In the current work, the Genetic Algorithm is
used as the method of the second phase, but any optimization technique
can be incorporated. In the last phase of the proposed technique,
a local minimization method is applied to the optimal parameters identified
in the second phase. The purpose of the present technique is first
to identify a reliable range of values for the parameters of RBF networks
...
clustering technique such as the K-Means technique is applied to identify
a promising range of values for the model parameters. In the second
stage, a global optimization algorithm is used to minimize the error
function inside the bounds of the first phase and in the last phase
a local minimization method will be used to find a guaranteed local
minimum of the error function. The method of the last phase guarantees
the training of the parameters within the range of values of the first
phase.
...
the training of neural networks and the efficient adaptation of parameters.
Among them, one can find methods aimed to efficient initialize the
parameters of RBF networks \citep{rbfinit1,rbfinit2,rbfinit3}. Moreover,
Benoudjit et al. provided a discussion on the estimation of kernel
widths in these models \citep{rbfkernel}. Additionally, a series
of pruning techniques \citep{rbfprun1,rbfprun2,rbfprun3} have been
introduced aiming to reduce the required number of parameters for
networks, providing a solution to the overfitting problem. Also, methods
that construct the architecture of RBF networks have been proposed
recently, such as the work of Du et al. \citep{rbfcon1} or the work
of Yu et al. who suggested an incremental design framework for RBF
networks \citep{rbfcon2}. Also, a series of optimization techniques
have been used in the past for the minimization of equation \ref{eq:RbfError},
such as genetic algorithms \citep{rbfga1,rbfga2}, the Particle Swarm
Optimization method \citep{rbfpso1,rbfpso2}, the Differential Evolution
technique \citep{rbfdiff1} etc. Furthermore, the rapid increase in
the use of parallel computing techniques in recent decades has resulted
in the publication of a series of relevant scientific papers that
exploit these techniques \citep{rbfpar1,rbfpar2}. Recently, Rani
...
\begin{algorithm}[H]
\caption{The steps of the K-means algorithm.\protect\label{alg:The-K-Means-algorithm.}}
...
Figure \ref{fig:The-layout-of} outlines the layout of the chromosomes
participated in the genetic algorithm of this phase. In this figure
...
\par\end{raggedright}
\caption{This image shows the schematic arrangement of a chromosome for the
proposed genetic algorithm.\protect\label{fig:The-layout-of}}
\end{figure}

The proposed genetic algorithm consists of the following steps:
\begin{enumerate}
...
}The remaining individuals will be substituted by offrpsings produced
...
\item \textbf{If} $k<N_{g}$ then the procedure continues from the Fitness
Calculation Step
\item \textbf{Else return }the best located chromosome $g^{*}$ as the result
of this algorithm.
\end{enumerate}
\end{enumerate}
...
In the third phase of the current work, a local optimization procedure
...
local optimization method. This variant can preserve the bounds located
...
The classification datasets that was incorporated in the conducted
experiments accompanied with their information (patterns and number
of classes) are presented in Table \ref{tab:classData}.

\begin{table}[H]
\caption{The column DATASET denotes the name of the dataset. The column entitled
Reference Paper denotes the paper, where this dataset was mentioned.
The column Patterns stands for the number of patterns in the dataset
and the column Number of Classes represents the number of distinct
classes in the mentioned dataset.\protect\label{tab:classData}}
...
The computation environment for the implemented series of experiments
was a Debian Linux system equipped with 128GB of RAM and all the necessary
code was written in the C++ programming language. Also, the OPTIMUS
computing environment \citep{optimus}, available from \url{https://github.com/itsoulos/GlobalOptimus.git}
...
validation technique. The average classification error is calculated
as: 
\begin{equation}
...
\item The results from the application of the ADAM local optimization technique
\citep{Adam,AdamNN} to train a neural network with 10 weights are
presented in the column ADAM.
...
\item The column BAYES stands for the application of the Bayesian optimizer
as implemented in the freely available library BayesOpt \citep{bayesopt}.
This method was incorporated to train a neural network with 10 processing
nodes.
...
\caption{Statistical analyses of the experimental results on the classification
datasets using the machine learning methods described in this article.\protect\label{fig:statClass}}
\end{figure}
...
\end{tabular}
\par\end{centering}
\caption{Comparison of precision and recall between the original RBF training
method and the proposed method.\protect\label{tab:Precision-and-recall}}
\end{table}
\begin{table}[H]
\caption{The experimental results obtained from the regression datasets, employing
the machine learning techniques presented in this work.\protect\label{tab:expRegression}}
...
\begin{table}[H]
\caption{Experimental results obtained by applying the proposed method to the
classification datasets, with the critical parameter $F$ ranging
from 1 to 8.\protect\label{tab:expClassF}}
...
\begin{table}[H]
\caption{Experimental results obtained by applying the proposed method to the
regression datasets, with the critical parameter $F$ ranging from
1 to 8.\protect\label{tab:expRegressionF}}
...
\caption{Statistical analysis of the results produced by the proposed approach
on the classification datasets, with variations in the parameter $F$.\protect\label{fig:statClassF}}
...
\begin{table}[H]
\caption{Experiments conducted on the classification datasets using the proposed
method, employing various strategies for computing the $\sigma$ parameters
in the radial basis functions.\protect\label{tab:expClassSigma}}
...
\begin{table}[H]
\caption{Results derived by the incorporation of the proposed method on the
regression datasets with different calculation mechanisms for the
$\sigma$ used in the radial functions.\protect\label{tab:expRegressionSigma}}
...
\caption{Statistical analysis of the results produced by applying the proposed
method to the classification datasets, considering various computation
techniques for estimating the range of $\sigma$ values in the radial
basis functions.\protect\label{fig:statClassSigma}}
...
techniques for the range of $\sigma$ values in the radial functions.\protect\label{fig:statRegressionSigma}}
...
\begin{table}[H]
\caption{Results obtained by applying the proposed method to the classification
datasets, while varying the number of generations $N_{g}$ \LyXZeroWidthSpace{}
between 50 and 400. \protect\label{tab:expClassNG}}
...
\caption{Statistical analysis of the results from the application of the proposed
method to the classification datasets, considering different values
of the parameter $N_{g}$. \protect\label{fig:statClassNG}}
...
\caption{Statistical analysis of the results from the application of the proposed
method to the regression datasets, considering different values of
the parameter $N_{g}$. \protect\label{fig:statRegressionNG}}
...
this time can be minimized with the incorporation of modern parallel
computing techniques like OpenMP \citep{gen_openmp} or MPI \citep{gen_mpi}.
